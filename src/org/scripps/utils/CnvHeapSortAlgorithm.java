 ...
 * @author gerikson
 */
public class CnvHeapSortAlgorithm extends JFrame implements Runnable
{

	public static String selection;
	public static double[][] ArrayforSort;
	public static Object[][] SortedData;
	public static ArrayList<CnvReader> arrayOfLines;
	...
	public CnvHeapSortAlgorithm(ArrayList<CnvReader> al, int d, String sel)
	{
		arrayOfLines = al;
		dataLines = d;
		selection = sel;
	...
	public static void inputData(ArrayList<CnvReader> arrayOfLines2, int dataL,
			String sel)
	{
		...
		// array to be sorted, 2 columns 1 is column number and second is value
		// that needs sorting
		...
		for (int s = 0; s < CnvShowTable.columns.length; s++)
		{
			if (selection == CnvShowTable.columns[s])
			{
				columnNumber = s;
			}
		...
		for (int j = 0; j < dataLines; j++)
		{
			...
			String lines = arrayOfLines2.get(j).fileRow;
		...
		for (int i = ArrayforSort.length; i > 0; i--)
		{
			...
			if (datacount == 500)
			{
				...
				CnvShowTable.createContentTable(SortedData,
						CnvShowTable.columns, "Sorted File "
								+ CnvReadFile.fileName + " by column: "
								+ selection);
				// delete FilterTable frame if exists
				/*
				 * if (FilterTable.DEBUG == true) { FilterTable.frame.dispose();
				 * }
				 */
				CnvShowTable.tableStatus = 2;
				CnvHeapSortAlgorithm.frame.dispose();
		...
		if (datacount < 500)
		{
			...
			String lin;
			SortedData = new Object[datacount][CnvShowTable.columns.length];
			begin = dataLines - 1;
			last = dataLines - 501;
			for (int ij = ArrayforSort.length - 1; ij > ArrayforSort.length
					- datacount + 1; ij--)
			{
				int value = (int) ArrayforSort[ij][0];
				lin = arrayOfLines2.get(value).fileRow;
			...
			progressBar.setStringPainted(true);
			CnvShowTable.frame.dispose();
			CnvShowTable.createContentTable(SortedData, CnvShowTable.columns,
	...
	public static void PreviousPage()
	{
		...
		String lin;
		Object[][] SortedDat;
		SortedDat = new Object[500][CnvShowTable.columns.length];
		...
		CnvShowTable.frame.dispose();
		CnvShowTable.createContentTable(SortedDat, CnvShowTable.columns,
				"Sorted File " + CnvReadFile.fileName + " by column: "
						+ selection);
	...
	public static void SortNext() throws InterruptedException
	{
		...
		} else if (datacount < (dataLines - last + 500) && last > 0
				&& last - 500 > 0)
		{
		...
		else
		{
			...
			if (tr == false)
			{
				...
				String lin;
				Object[][] SortedDat;
				SortedDat = new Object[begin][CnvShowTable.columns.length];
				...
				CnvShowTable.frame.dispose();
				// ShowTable.createContentTable(SortedDat, headArray,
				// "Sorted File " + ReadFile.fileName + " by column: " +
				// selection + " Last Page!");
				CnvShowTable.createContentTable(SortedDat,
						CnvShowTable.columns, "Sorted File "
								+ CnvReadFile.fileName + " by column: "
								+ selection);
				tr = true;
	...
	public void run()
	{
		...
		try
		{
			...
			// List<Runnable> a = (List<Runnable>)
			// ShowTable.threadExecutor.shutdownNow();
