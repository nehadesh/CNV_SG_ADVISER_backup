...
public class CnvStatistics implements Runnable
{
	...
	// private int TFBS = 0;
	// not sure what this is used for, all of the muRNA counter that are > 0 or
	// value "DELETED"
	...
	private void calculateStatsFile(File file) throws IOException
	{
		...
		while ((line = bReader.readLine()) != null)
		{
			...
			/*
			 * int[] i = Polythen(spLine); int b = Coding_Impact(spLine); int c
			 * = calcStats(spLine); int d = Sift(spLine); int e =
			 * Condel(spLine); int f = Truncation(spLine); /* if either column a
			 * Functional variant I add to total disrupt only once
			 */
			/*
			 * if (i[0]==1 || b ==1 || c == 1) { TOT_DISRUPT_NEW ++; }
			 * 
			 * if (i[1]==1||d==1 || e == 1 || temp_splice || PFAM || f==1) {
			 * TOT_POSIB_DISRUPT++; temp_splice = false; PFAM = false; }
			 */
			...
			/*
			 * if either column a Functional variant I add to total disrupt only
			 * once
			 */
			...
			throws IOException
	{
		...
		for (int i = 0; i < TempArray.size(); i++)
		{
			...
			/*
			 * polythen(spLine); Coding_Impact(spLine); calcStats(spLine);
			 */
			int[] a = Polythen(spLine);
			int b = Coding_Impact(spLine);
			int c = calcStats(spLine);
	...
	private int calcStats(String[] spLine)
	{
		int i = 0;
		/*
		 * counting SNPs Nonsense -> Nonsynonymous -> Synonymous -> 5UTR or 3UTR
		 * ->Intron
		 */
		if (spLine[getHeaderIndex("VarType")].contains("snp"))
		{
			...
			if (spLine[getHeaderIndex("Coding_Impact")].contains("Nonsense"))
			{
				coding_snp++;
				nonsense_snp++;
			} else if (spLine[getHeaderIndex("Coding_Impact")]
					.contains("Nonsynonymous"))
			{
				coding_snp++;
				nonsyn_snp++;
			} else if (spLine[getHeaderIndex("Coding_Impact")]
					.contains("Synonymous"))
			{
				coding_snp++;
				syn_snp++;
			} else if (spLine[getHeaderIndex("Location")].contains("5UTR")
					|| spLine[getHeaderIndex("Coding_Impact")].contains("3UTR"))
			{
				UTR_snp++;
		...
		/*
		 * DELETIONS/INSERTIONS Frameshift ->In_Frame_Rearrangement ->5UTR or
		 * 3UTR ->Intro
		 */
		else if (spLine[getHeaderIndex("VarType")].contains("delins"))
		{
			...
			if (spLine[getHeaderIndex("Coding_Impact")].contains("Frameshift"))
			{
				...
				fshift_rear++;
					...
					.contains("In_Frame_Rearrangement"))
			{
				in_frame_rear++;
				coding_rear++;
			} else if (spLine[getHeaderIndex("Location")].contains("5UTR")
					|| spLine[getHeaderIndex("Location")].contains("3UTR"))
			{
				UTR_rear++;
			} else if (spLine[getHeaderIndex("Location")].contains("Intron"))
			{
		...
		/*
		 * INSERTIONS Frameshift -> InterCodon_In_Frame_Insertion
		 * ->In_Frame_Insertion ->5UTR or 3UTR ->Intron
		 */
		...
		else if (spLine[getHeaderIndex("VarType")].contains("ins"))
		{
			...
			if (spLine[getHeaderIndex("Coding_Impact")].contains("Frameshift"))
			{
				...
				fshift_ins++;
					...
					.contains("InterCodon_In_Frame_Insertion"))
			{
				out_frame_ins++;
				coding_ins++;
			} else if (spLine[getHeaderIndex("Coding_Impact")]
					.contains("In_Frame_Insertion"))
			{
				in_frame_ins++;
				coding_ins++;
			} else if (spLine[getHeaderIndex("Location")].contains("5UTR")
		...
		/*
		 * DELETIONS Frameshift ->In_Frame_Deletion_One_Altered_Codon
		 * ->In_Frame_Deletion->5UTR or 3UTR ->Intron
		 */
		else if (spLine[getHeaderIndex("VarType")].contains("del"))
		{
			...
			if (spLine[getHeaderIndex("Coding_Impact")].contains("Frameshift"))
			{
				...
				fshift_del++;
					...
					.contains("In_Frame_Deletion_One_Altered_Codon"))
			{
				inter_codon_del++;
				coding_del++;
			} else if (spLine[getHeaderIndex("Coding_Impact")]
					.contains("In_Frame_Deletion"))
			{
				in_frame_del++;
				coding_del++;
			} else if (spLine[getHeaderIndex("Location")].contains("5UTR")
	...
	private void writeToFile() throws IOException
	{
		...
		outfile.write("Coding SNPs:\t" + "\t" + Integer.toString(coding_snp)
				+ "\n");
		outfile.write("Nonsynonymous SNPs:\t" + "\t"
				+ Integer.toString(nonsyn_snp) + "\n");
		outfile.write("Synonymous SNPs:\t" + "\t" + Integer.toString(syn_snp)
				+ "\n");
		outfile.write("Nonsense SNPs:\t" + "\t"
				+ Integer.toString(nonsense_snp) + "\n");
		outfile.write("Untranslated Region SNPs:\t" + "\t"
				+ Integer.toString(UTR_snp) + "\n");
		// outfile.write("Non-coding RNA SNPs:\t" + "\t" + Integer.toString +
		// "\n")
		outfile.write("Intronic SNPs:\t" + "\t" + Integer.toString(intron_snp)
				+ "\n");
		outfile.write("Damaging Truncations SNPs:\t" + "\t"
				+ Integer.toString(trunc_snp) + "\n");
		// outfile.write("Intergenic SNPs:\t" + "\t".join(str(x) for x in
		// ingenic_snp_tracker) + "\n")
		outfile.write("Total SNPs:\t" + "\t" + Integer.toString(tot_snp) + "\n");

		outfile.write("Coding Insertions:\t" + "\t"
				+ Integer.toString(coding_ins) + "\n");
		outfile.write("In-frame Insertions:\t" + "\t"
				+ Integer.toString(in_frame_ins) + "\n");
		outfile.write("Out-of-frame Insertions:\t" + "\t"
				+ Integer.toString(out_frame_ins) + "\n");
		outfile.write("Frameshift Insertions:\t" + "\t"
				+ Integer.toString(fshift_ins) + "\t" + "\n");
		outfile.write("Untranslated region Insertions:\t" + "\t"
				+ Integer.toString(UTR_ins) + "\n");
		// outfile.write("Non-coding RNA Insertions:\t" + "\t" +
		// Integer.toString() + "\n")
		outfile.write("Intronic Insertions:\t" + "\t"
				+ Integer.toString(intron_ins) + "\n");
		outfile.write("Damaging Truncations Insertions:\t" + "\t"
				+ Integer.toString(trunc_ins) + "\n");
		// outfile.write("Intergenic Insertions:\t" + "\t" +
		// Integer.toString(ingenic_ins) + "\n");
		outfile.write("Total Insertions:\t" + "\t" + Integer.toString(tot_ins)
				+ "\n");

		outfile.write("Coding Deletions:\t" + "\t"
				+ Integer.toString(coding_del) + "\n");
		outfile.write("In-frame Deletions:\t" + "\t"
				+ Integer.toString(in_frame_del) + "\n");
		outfile.write("Inter-Codon Deletions:\t" + "\t"
				+ Integer.toString(inter_codon_del) + "\n");
		outfile.write("Frameshift Deletions:\t" + "\t"
				+ Integer.toString(fshift_del) + "\n");
		outfile.write("Untranslated region Deletions:\t" + "\t"
				+ Integer.toString(UTR_del) + "\n");
		// outfile.write("Non-coding RNA Deletions:\t" + "\t"+
		// Integer.toString(noncoding_del) + "\n");
		outfile.write("Intronic Deletions:\t" + "\t"
				+ Integer.toString(intron_del) + "\n");
		outfile.write("Damaging Truncations deletions:\t" + "\t"
				+ Integer.toString(trunc_del) + "\n");
		// outfile.write("Intergenic Deletions:\t" + "\t" +
		// Integer.toString(ingenic_del) + "\n");
		outfile.write("Total Deletions:\t" + "\t" + Integer.toString(tot_del)
				+ "\n");

		outfile.write("Coding rearrangements:\t" + "\t"
				+ Integer.toString(coding_rear) + "\n");
		outfile.write("In-frame rearrangements:\t" + "\t"
				+ Integer.toString(in_frame_rear) + "\n");
		outfile.write("Frameshift rearrangements:\t" + "\t"
				+ Integer.toString(fshift_rear) + "\n");
		outfile.write("Untranslated region rearrangements:\t" + "\t"
				+ Integer.toString(UTR_rear) + "\n");
		// outfile.write("Non-coding RNA rearrangements:\t" + "\t" +
		// Integer.toString(noncoding_rear) + "\n")
		outfile.write("Intronic rearrangements:\t" + "\t"
				+ Integer.toString(intron_rear) + "\n");
		outfile.write("Damaging Truncations rearangements:\t" + "\t"
				+ Integer.toString(trunc_rear) + "\n");
		// outfile.write("Intergenic rearrangements:\t" + "\t" +
		// Integer.toString(ingenic_rear) + "\n");
		outfile.write("Total rearrangements:\t" + "\t"
				+ Integer.toString(tot_rear) + "\n");
		...
		if (coding_snp != 0)
		{
			...
			outfile.write("Rate Nonsense SNPs:\t" + "\t"
					+ String.format("%.5g%n", test) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		}
		outfile.write("Frameshift Structural Variants:\t" + "\t"
				+ (fshift_del + fshift_ins + fshift_rear) + "\n");
		 ...
		 * (coding_ins != 0) { double d = (double) fshift_ins/coding_ins;
		 * outfile.write("Rate:\t" + String.format("%.5g%n",d) + "\n"); } else {
		 * outfile.write("Rate:\tN/A\n"); }
		 * outfile.write("Frameshift Deletions:\t" + fshift_del + "\n"); if
		 * (coding_del != 0) { double d = (double) fshift_del/coding_del;
		 * outfile.write("Rate:\t" + String.format("%.5g%n", d) + "\n"); } else
		 * { outfile.write("Rate:\tN/A\n"); }
		 * outfile.write("Frameshift Rearangements:\t" + fshift_rear + "\n"); if
		 * (coding_rear != 0) { double d = (double) fshift_rear/coding_rear;
		 * outfile.write("Rate:\t" + String.format("%.5g%n", d) + "\n"); } else
		 * { outfile.write("Rate:\tN/A\n"); }
		 */
		...
		outfile.write("Protein motif damaging Variants:\t" + "\t"
				+ pmotif_major + "\n");
		if (pmotif != 0)
		{
		...
		outfile.write("Protein Impact Prediction(SIFT INTOLERANT) Variants:\t"
				+ "\t" + SIFT_INTOLERANT + "\n");
		outfile.write("Protein Impact Prediction(SIFT TOLERANT) Variants:\t"
				+ "\t" + SIFT_TOLERANT + "\n");
		outfile.write("Protein Impact Prediction(Condel deletirious) Variants:\t"
				+ "\t" + Condel_deletirious + "\n");
		outfile.write("Protein Impact Prediction(Condel neutral) Variants:\t"
				+ "\t" + Condel_neutral + "\n");

		outfile.write("TFBS Variants:\t" + "\t"
				+ (TFBS_snp_tot + TFBS_ins_tot + TFBS_del_tot + TFBS_rear_tot)
				+ "\n\n");
		outfile.write("TFBS Disrupting Variants:\t" + "\t"
				+ (TFBS_snp + TFBS_ins + TFBS_del + TFBS_rear) + "\n\n");
		int a = TFBS_snp_tot + TFBS_ins_tot + TFBS_del_tot + TFBS_rear_tot;
		if (a != 0)
		{
			double d = (double) (TFBS_snp + TFBS_ins + TFBS_del + TFBS_rear)
					/ a;
			outfile.write("Rate:\t" + "\t" + String.format("%.5g%n", d) + "\n");
		} else
		{
		...
		outfile.write("pre-miRNA Disrupting Variants:\t" + "\t"
				+ premiRNA_disrupt + "\n");
		...
		int miRNA_tot = miRNA_snp_tot + miRNA_del_tot + miRNA_ins_tot
				+ miRNA_rear_tot;
		outfile.write("miRNA-BS Variants:\t" + "\t" + miRNA_tot + "\n");
		outfile.write("miRNA-BS Disrupting Variants:\t"
				+ "\t"
				+ (miRNA_snp_del + miRNA_del_del + miRNA_ins_del + miRNA_rear_del)
				+ "\n");
		if (miRNA_tot != 0)
		{
			double t = (double) (miRNA_snp_del + miRNA_del_del + miRNA_ins_del + miRNA_rear_del)
					/ miRNA_tot;
			outfile.write("Rate:\t" + "\t" + String.format("%.5g%n", t) + "\n");
		} else
		{
		...
		outfile.write("miRNA-BS CREATED Variants:\t"
				+ "\t"
				+ (miRNA_snp_created + miRNA_del_created + miRNA_ins_created + miRNA_rear_created)
				+ "\n");
		if (miRNA_tot != 0)
		{
			double t = (double) (miRNA_snp_created + miRNA_del_created
					+ miRNA_ins_created + miRNA_rear_created)
					/ miRNA_tot;
			outfile.write("Rate:\t" + "\t" + String.format("%.5g%n", t) + "\n");
		} else
		{
		...
		outfile.write("miRNA-BS CHANGED Variants:\t"
				+ "\t"
				+ (miRNA_snp_changed + miRNA_del_changed + miRNA_ins_changed + miRNA_rear_changed)
				+ "\n");
		if (miRNA_tot != 0)
		{
			double t = (double) (miRNA_snp_changed + miRNA_del_changed
					+ miRNA_ins_changed + miRNA_rear_changed)
					/ miRNA_tot;
			outfile.write("Rate:\t" + "\t" + String.format("%.5g%n", t) + "\n");
		} else
		{
		...
		outfile.write("miRNA genomic Variants:\t"
				+ "\t"
				+ (miRNA_genomic_snp + miRNA_genomic_del + miRNA_genomic_ins + miRNA_genomic_rear)
				+ "\n\n");

		outfile.write("Total Likely Functional Variants:\t" + "\t"
				+ TOT_DISRUPT + "\n");
		if (datacount != 0)
		{
		...
		outfile.write("Total Potential Functional Variants:\t" + "\t"
				+ TOT_DISRUPT_NEW + "\n");
		if (datacount != 0)
		{
		...
		if (tot_snp != 0)
		{
			...
			outfile.write("Rate pre-miRNA Disrupting SNPs:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		}
		...
		if (splice_snp_tot != 0)
		{
			...
			outfile.write("Rate Splicing Change SNPs:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		...
		if (pmotif_snp_tot != 0)
		{
			...
			outfile.write("Rate Protein Motif disrupting SNPs:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		...
		if (TFBS_ins_tot != 0)
		{
			...
			outfile.write("Rate TFBS Insertions:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		}
		outfile.write("pre-miRNA Disrupting Insertions:\t" + "\t"
				+ premiRNA_ins_tot + "\n");
		if (tot_ins != 0)
		{
			double d = (double) premiRNA_ins_tot / tot_ins;
			outfile.write("Rate pre-miRNA Disrupting Insertions:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		...
		outfile.write("miRNA-BS disrupting Insertions:\t" + "\t"
				+ miRNA_ins_del + "\n");
		if (miRNA_ins_tot != 0)
		{
		...
		outfile.write("miRNA-BS CREATED Insertions:\t" + "\t"
				+ miRNA_ins_created + "\n");
		if (miRNA_ins_tot != 0)
		{
		...
		outfile.write("miRNA-BS CHANGED Insertions:\t" + "\t"
				+ miRNA_ins_changed + "\n");
		if (miRNA_ins_tot != 0)
		{
		...
		if (splice_ins_tot != 0)
		{
			...
			outfile.write("Rate Splicing Change Insertions:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		}
		outfile.write("Protein Motif disrupting Insertions:\t" + "\t"
				+ pmotif_ins + "\n");
		if (pmotif_ins_tot != 0)
		{
			double d = (double) pmotif_ins / pmotif_ins_tot;
			outfile.write("Rate Protein Motif disrupting Insertions:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		...
		outfile.write("pre-miRNA Disrupting Deletions:\t" + "\t"
				+ premiRNA_del_tot + "\n");
		if (tot_del != 0)
		{
			double d = (double) premiRNA_del_tot / tot_del;
			outfile.write("Rate pre-miRNA Disrupting Deletions:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		...
		outfile.write("miRNA-BS CREATED Deletions:\t" + "\t"
				+ miRNA_del_created + "\n");
		if (miRNA_del_tot != 0)
		{
		...
		outfile.write("miRNA-BS CHANGED Deletions:\t" + "\t"
				+ miRNA_del_changed + "\n");
		if (miRNA_del_tot != 0)
		{
		...
		if (splice_del_tot != 0)
		{
			...
			outfile.write("Rate Splicing Change Deletions:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		}
		outfile.write("Protein Motif disrupting Deletions:\t" + "\t"
				+ pmotif_del + "\n");
		if (pmotif_del_tot != 0)
		{
			double d = (double) pmotif_del / pmotif_del_tot;
			outfile.write("Rate Protein Motif disrupting Deletions:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		...
		if (TFBS_rear_tot != 0)
		{
			...
			outfile.write("Rate TFBS Rearangements:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		}
		outfile.write("pre-miRNA Disrupting Rearangements:\t" + "\t"
				+ premiRNA_rear_tot + "\n");
		if (tot_rear != 0)
		{
			double d = (double) premiRNA_rear_tot / tot_rear;
			outfile.write("Rate pre-miRNA Rearangements:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		...
		outfile.write("miRNA-BS disrupting Rearangements:\t" + "\t"
				+ miRNA_rear_del + "\n");
		if (miRNA_rear_tot != 0)
		{
		...
		outfile.write("miRNA-BS CREATED Rearangements:\t" + "\t"
				+ miRNA_rear_created + "\n");
		if (miRNA_rear_tot != 0)
		{
		...
		outfile.write("miRNA-BS CHANGED Rearangements:\t" + "\t"
				+ miRNA_rear_changed + "\n");
		if (miRNA_rear_tot != 0)
		{
		...
		outfile.write("miRNA genomic Rearangements:\t" + "\t"
				+ miRNA_genomic_rear + "\n\n");
		...
		if (splice_rear_tot != 0)
		{
			...
			outfile.write("Rate Splicing Change Rearangements:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
		}
		outfile.write("Protein Motif disrupting Rearangements:\t" + "\t"
				+ pmotif_rear + "\n");
		if (pmotif_rear_tot != 0)
		{
			double d = (double) pmotif_rear / pmotif_rear_tot;
			outfile.write("Rate Protein Motif disrupting Rearangements:\t"
					+ String.format("%.5g%n", d) + "\n");
		} else
		{
			outfile.write("Rate:\tN/A\n");
	...
	public boolean TFBS_calc(String[] spLine)
	{
		...
		for (int i = 0; i < entry.length; i++)
		{
			if (entry[i].equals("DELETED"))
			{
				...
				/*
				 * if (CONS_FOUND &&
				 * spLine[getHeaderIndex("Location")].contains("Upstream")) {
				 * lowest = -40; }
				 */
		...
		// calculate only the major TFBS variation that are smaller then -7, are
		// upstreap and is a conserved variants;
		// if (CONS_FOUND &&
		// spLine[getHeaderIndex("Location")].contains("Upstream")) {
	...
	private boolean CONS_FOUND_Fuction(String[] spLine)
	{
		...
		if (!spLine[getHeaderIndex("Conserved46way")].equals("-")
				&& !spLine[getHeaderIndex("Conserved46way")].startsWith("N/A"))
		{
			CONS_FOUND = true;
			cons_count++;
		}
		// phastConsElements46wayPlacental
		else if (!spLine[getHeaderIndex("Conserved46wayPlacental")].equals("-")
				&& !spLine[getHeaderIndex("Conserved46wayPlacental")]
						.startsWith("N/A"))
		{
			CONS_FOUND = true;
			cons_count++;
		...
		else if (!spLine[getHeaderIndex("Conserved46wayPrimates")].equals("-")
				&& !spLine[getHeaderIndex("Conserved46wayPrimates")]
						.startsWith("N/A"))
		{
			CONS_FOUND = true;
			cons_count++;
	...
	/*
	 * Count all of the values in the miRNA_BS_deltaG, that will be the total
	 * number of miRNA
	 */
	private int miRNA(String[] spLine)
	{
		// extract the value in the miRNA_BS_deltaG column and make sure it's
		// not empty
		...
		for (int i = 0; i < entry.length; i++)
		{
			...
			if (!entry[i].equals("-"))
			{
				...
				if (spLine[getHeaderIndex("miRNA_BS_impact")]
						.contains("DELETED"))
				{
					TOT_DISRUPT++;
					return 1;
				} else if (test < 0.0)
				{
					TOT_DISRUPT++;
					return 1;
				} else if (spLine[getHeaderIndex("miRNA_BS_impact")]
						.contains("CREATED"))
				{
					return 2;
				}
	...
	{
		// extract the value in the miRNA_genomic column and make sure it's not
		// empty
	...
	{
		int i = 0;
		if (spLine[getHeaderIndex("Trunc_Prediction")]
				.contains("Damaging_Truncation"))
		{
			i = 1;
		}
		return i;
	...
	{
		int[] i = new int[2];
		if (spLine[getHeaderIndex("Protein_Impact_Prediction(Polyphen)")]
				.contains("probably damaging"))
		{
			TOT_DISRUPT++;
			probably_damaging++;
			i[0] = 1;
			i[1] = 1;
		} else if (spLine[getHeaderIndex("Protein_Impact_Prediction(Polyphen)")]
				.contains("possibly damaging"))
		{
			TOT_DISRUPT++;
			possibly_damaging++;
			i[0] = 1;
	...
	{
		int i = 0;
		if (spLine[getHeaderIndex("Protein_Impact_Prediction(SIFT)")]
				.contains("INTOLERANT"))
		{
			SIFT_INTOLERANT++;
			i = 1;
		} else if (spLine[getHeaderIndex("Protein_Impact_Prediction(SIFT)")]
				.contains("TOLERANT"))
		{
			SIFT_TOLERANT++;
			// i =1;
	...
	{
		int i = 0;
		if (spLine[getHeaderIndex("Protein_Impact_Prediction(Condel)")]
				.contains("deleterious"))
		{
			Condel_deletirious++;
			i = 1;
		} else if (spLine[getHeaderIndex("Protein_Impact_Prediction(Condel)")]
				.contains("neutral"))
		{
			Condel_neutral++;
			// i =1;
	...
	private boolean premiRNA(String[] spLine)
	{
		...
		String[] entry = spLine[getHeaderIndex("miRNA_folding_deltaG")]
				.split("///");
		for (int i = 0; i < entry.length; i++)
		{
			if (!entry[i].equals("-") && !entry[i].contains("N/A"))
			{
				if (entry[i].contains("DELETED")
						|| Float.parseFloat(entry[i]) < 0)
				{
					TOT_DISRUPT++;
					premiRNA_disrupt++;
					premiRNA_found = true;
		...
		/*
		 * check to make sure that premiRNA wasn't found yet, if it wasn't found
		 * check the column miRNA_binding_deltaG
		 */
		if (!premiRNA_found)
		{
			String[] entryTwo = spLine[getHeaderIndex("miRNA_binding_deltaG")]
					.split("///");
			for (int i = 0; i < entry.length; i++)
			{
				if (!entryTwo[i].equals("-") && !entryTwo[i].contains("N/A"))
				{
					if (entryTwo[i].contains("DELETED")
							|| Float.parseFloat(entry[i]) < 0)
					{
						TOT_DISRUPT++;
						premiRNA_disrupt++;
	...
	private boolean splicingChange(String[] spLine)
	{
		...
		String[] entry = spLine[getHeaderIndex("Splicing_Prediction(MaxENT)")]
				.split("///");
		for (int i = 0; i < entry.length; i++)
		{
			if (entry[i].equals("-"))
	...
	private boolean Protein_Domains_Impact(String[] spLine)
	{
		...
		String[] entry = spLine[getHeaderIndex("Protein_Domains_Impact(LogRE)")]
				.split("///");
		for (int i = 0; i < entry.length; i++)
		{
			if (entry[i].equals("-"))
