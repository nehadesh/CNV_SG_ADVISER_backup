 ...
 * @author gerikson
 */
public class CnvOpenCG
{

	public static javax.swing.JFileChooser fileChooser;
	public static CnvViewerInterface CGviewFiftyLines;
	...
	public void openFile() throws FileNotFoundException, IOException
	{
		...
		int returnVal = fileChooser.showOpenDialog(CnvShowTable.frame);
		...
		if (returnVal == JFileChooser.APPROVE_OPTION)
		{
			...
			} else
			{
				...
				System.out
						.println("You are attempting to load in a file that is not compressed, ATTN: if the file doesn't end in 'tsv' this might not be a Complete Cenomics file!");
				...
				try
				{
					...
					while ((line = bReader.readLine()) != null)
					{
						...
						else if (line.startsWith(">", 0))
						{
							...
							// if chromosome is on the 3rd position this is a
							// masterVar file
							if (headSplit[2].contains("chromosome"))
							{
								System.out
										.println("This is a master var file.");
								masterVar = true;
							}
							continue;
						...
						// what is in varType if it's junk we don't have to
						// bother comparing it more...
						String[] tempLin = line.split("\t");

						if (tempLin[6].contains("no-call")
								|| tempLin[6].contains("no-ref")
								|| tempLin[6].contains("complex")
								|| tempLin[6].contains("ref")
								|| tempLin[6].contains("PAR"))
						{
							...
							{
								String anLine = CnvReadFile.arrayOfLines
										.get(mainFileRowCount).fileRow;
								String mLine = deleteNA(anLine);
								String newLine = haplotype.concat("\t").concat(
										mLine);
								CnvReadFile.arrayOfLines.get(mainFileRowCount).fileRow = newLine;
								mainFileRowCount++;
								haplotype = "00";
								continue;
							} else
						...
						if (mainFileRowCount < CnvReadFile.arrayOfLines.size())
						{
							if (mainFileRowCount % 10000 == 0)
							{
								...
								System.out.println(dateFormat.format(cal
										.getTime()));
								System.out
										.println("Missing lines in missingLines array: "
												+ missingLines.size());
				...
				int end;
				if (CnvReadFile.arrayOfLines.size() > 1000)
				{
					end = 1000;
				} else
				{
				...
				CnvShowTable.tableStatus = -1;
				CnvShowTable.into2DArray(TempArray, end);
				...
				String one = "Lines not found in the annotated file is: "
						+ missingLinesAnnotated;
				JLabel label1 = new JLabel(one);
				demo1.frame.add(label1);
				String two = "Lines not found in the input file is: "
						+ missingLinesInput;
		...
		System.out.println("Line not found int the annotated file is: "
				+ missingLinesAnnotated);
		for (int i = 0; i < missingLines.size(); i++)
		{
			System.out.println(missingLines.get(i));
		}
		System.out.println("Line not found in the input file is: "
				+ missingLinesInput);
	}
	...
	public String[] lineTransform(String line)
	{
		...
		} else
		{
			try
			{
				...
				// if it's not an integer for chromosome throw exception and
				// kick us out
			...
			{
				System.out.println("Error! Non acceptable chromosome format: "
						+ lin[3]);
				String[] var = null;
				return var;
			}
		...
		{
			lin[6] = "delins";
		} else if (lin[6].equals("del"))
		{
			lin[8] = "-";
		} else if (lin[6].equals("ins"))
		{
			lin[7] = "-";
		}
	...
	public boolean compareAlgorithm(String line)
	{
		...
		if (mainFileRowCount < CnvReadFile.arrayOfLines.size())
		{
			String anLine = CnvReadFile.arrayOfLines.get(mainFileRowCount).fileRow;
			...
			if (lin[3].equals(mainLine[3]) && lin[4].equals(mainLine[4])
					&& lin[5].equals(mainLine[5]) && lin[6].equals(mainLine[6])
					&& lin[7].equals(mainLine[7]) && lin[8].equals(mainLine[8]))
			{
				// insideLoop = false;
				String mLine = deleteNA(anLine);
				...
				if (haplotype.equals("00"))
				{
					...
					haplotype = "01";
					// go to the next line of complete genomics to see if it's
					// heterozagous
					return true;
				}
				...
				{
					// we skiped the previous line register this line and go to
					// previous line to verify with next
					missingAnnotated = 0;
					haplotype = "11";
					String newL = haplotype + "\t" + mLine;
					CnvReadFile.arrayOfLines.get(mainFileRowCount).fileRow = newL;
					mainFileRowCount++;
					haplotype = "00";
					return true;
			...
			else
			{
				...
				} else if (!(missingLines.size() == 0))
				{
					...
					for (int i = 0; i < missingLines.size(); i++)
					{
						String[] lineMissing = lineTransform(missingLines
								.get(i));
					...
					// if one line from the missing lines was found remove it
					// from the missing line array
					if (answ == true)
					{
					...
					} else
					{
						...
						if (Integer.parseInt(lin[3]) < Integer
								.parseInt(mainLine[3]))
						{
							missingLinesInput++;
							return true;
						} else if (Integer.parseInt(lin[3]) > Integer
								.parseInt(mainLine[3]))
						{
						...
						{
							mainFileRowCount++;
							// new thing, this deals with lines missing in the
							// annotated array - go to the next line of the
							// annotated array
							missingAnnotated++;
							if (missingAnnotated > 5)
							{
								...
								System.out.println("Skipping this line: "
										+ line);
								missingLinesAnnotated++;
								// keep it in the missingArray, highly doubt you
								// will find it
								return true;
							}
							// end of new thing
				...
				else
				{
							...
							.parseInt(mainLine[3]))
					{
						...
						System.out
								.println("Error! Variant not fount in the anotated file!");
					...
					/*
					 * Here is an interesting part, if the begin site is equal
					 * and the end, vartype etc is not therefore we might have
					 * the lines only they are sorted in the annotated file and
					 * not sorted in the CG file we need to keep this line in
					 * memory so we will be able to compare it on the next run.
					 */
